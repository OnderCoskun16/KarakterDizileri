...
namespace StringInceleme
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string cumle = "bu gün hava çok güzel....arkadaşlar ile kavak olan bir yere gittik. tabi gitmeden ütü yaptım. orada ata bindim.... kabak tatlısı yedik";
            ...
            foreach (var item in cumleler)
            {
                ...
                if (kelime.Length == 3)
                {
                    if (kelime[f] == kelime[l])
                    {
                        Console.WriteLine("Palindrom olan kelime =" + kelime);
                    }
                }
                else
                {
                    if (Lwd >= 3 && kelime[Fwd++] == kelime[Lwd--]) // k karşılaştır             
                        if (kelime[Fwd++] == kelime[Lwd--]) // a karşılaştır..
                        {
                            Console.WriteLine("Palindrom olan kelime =" + kelime);
                        }
                }
